...
General metrics calculated from the dataset. 
...
Time series that plots the amount of prescriptions vs the date
...
Distribution of the time of day when measurements were taken. Note that the time is represented in the GMT timezone and not up to date with the current timezone! 
...
ggplot(compliance_plot_total, aes(x=compliance, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 30)) + geom_text(aes(label = cnt), vjust = 1.5, colour = "white") + ggtitle("Distribution of user compliance") + ylab("Amount of patients")
