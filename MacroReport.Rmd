...
#configure input variables
    path <- file.path("/Volumes/GoogleDrive/Shared drives/9. Clinical/9.6 Data reporting/ZOL")
...
ggplot(plot_left_1_df, aes(x="", y=cnt, fill=gender)) +
  ...
  coord_polar("y", start=0) +  theme_void() + scale_fill_manual(values=c("#1e8d95", "#ffa786")) + geom_text(aes(label = cnt), position = position_stack(vjust = 0.5), color = "white", size=6)
...
ggplot(plot_right_1_df, aes(x=age, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + xlab("Age groups") + ylab("# subjects") + ggtitle("Distribution of age groups") + geom_text(aes(label = cnt), vjust = 1.5, colour = "white")
...
ggplot(plot_left_3_df, aes(x=wd, y=cnt_, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 90)) + scale_x_discrete(limits = plot_left_3_df$wd) + xlab("Week") + ylab("# of activatoins") + ggtitle("Activations on a weekly basis") + geom_text(aes(label = cnt_), size = 2, vjust = 1.5, colour = "white")
...
ggplot(plot_right_3_df, aes(x=hour_, y=cnt, fill="#1e8d95")) +
  ...
  scale_x_discrete(limits=plot_right_3_df$hour_)+ xlab("Hour of the day (GMT timezone)") + ylab("# measurements") + ggtitle("Measurements taken throughout the day")+ geom_text(aes(label = cnt), vjust = 1.5, colour = "white")
```
...
ggplot(compliance_plot_total, aes(x=compliance, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 30)) + geom_text(aes(label = cnt), vjust = 1.5, colour = "white")   
...
ggplot(plot_right_4_df, aes(x=age, y=per_user_cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + geom_text(aes(label = per_user_cnt), vjust = 1.5, colour = "white")
```
...
ggplot(plot_prescriptions_duration, aes(x=as.character(type), y=cnt)) +
  geom_bar(stat="identity", fill = "#1e8d95"  )+theme_minimal() + theme(legend.position = "none",  axis.text.x = element_text(angle = 45, vjust=0.7)) + geom_text(aes(label = cnt), vjust = -1, colour = "black") + xlab("Prescription duration") + ylab("Amount of prescriptions") + ylim(0, max(plot_prescriptions_duration$cnt)+30) + scale_x_discrete(limits = plot_prescriptions_duration$type)
...
pp <- get_sankey_plot(agg, min_num_users = 1, max_num_steps = 3, width = 900, 
                height = 1500, save = F)
#write.csv(agg,"/Users/larsgrieten/Dropbox (Qompium)/15 PRESENTATIONS/DATASET/agg.csv" )
print(pp)
```
...
```{r, message=FALSE, echo=FALSE}
  #Percentage of insufficient quality
percent_quality <-cast(measurments, userIds.0 ~ data.diagnosis.label.0)
percent_quality <-as.matrix(percent_quality)
percent_quality_2 <- as.data.frame(prop.table(percent_quality, 1)*100)

percent_quality_2<- percent_quality_2 %>%
select(userIds.0,quality_to_low)
  
total_percent_quality = glue::glue('{round(mean(percent_quality_2$quality_to_low),2)}%')
minimum_amount_quality = glue::glue('{round(min(percent_quality_2$quality_to_low),2)}%')
maximum_amount_quality = glue::glue('{round(max(percent_quality_2$quality_to_low),2)}%')
rbind(`Average % of insufficient quality:` = total_percent_quality, `Minimum % of insufficient quality:` = minimum_amount_quality ,`Maximum % of insufficient quality:` = maximum_amount_quality  ) %>% 
    kable() %>%
    kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
