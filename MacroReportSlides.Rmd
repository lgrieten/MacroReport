...
Oerview in time when patients activated their prescription.
...
ggplot(plot_left_3_df, aes(x=wd, y=cnt_, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 90)) + scale_x_discrete(limits = plot_left_3_df$wd) + xlab("Week") + ylab("# of measurements")  + geom_text(aes(label = cnt_), size = 2, vjust = 1.5, colour = "white")

```


## Measurement behavior
The timepoints during the day on which measurements are taken.Note that the time is GMT. 
...
- Users are asked to take `r m_per_day` measurements per day
...
## Average amount of measurements 

Average amount of measurements per age category. 
...
Overview metrics on diagnostic results. 
...
rbind(`Amount of people with Atrial Fibrillation:` = people_with_atrial_fibrillation
      ,`Amount of atrial fibrillation measurements ` = total_af_measurements
      ,`Amount of people with other cardiac arrhythmia's:` = people_with_other_cardiac_arrhythmias
      ...
      ,`AF-measurements with symptoms:` = scales::percent(1-(af_measurements_nosymptoms/total_af_measurements))
...
ggplot(plot_rhythm_symptom_correlation, aes(Rhythm, Symptoms)) + geom_point(aes(size = cnt), colour = "#1e8d95") +theme_bw() + scale_size_continuous(range=c(2,20)) + geom_text(aes(label = cnt),colour="white") + xlab("Rhythm Symptom correlation") + theme(legend.position = "none",  axis.text.x = element_text(angle = 90, vjust=0.7))

```

## Rhythm - Symptom table
...
kable(plot_rhythm_symptom_correlation2) %>%
  kable_styling(
    bootstrap_options = c("hover", "condensed", "responsive"),
    full_width = F,
    font_size = 14)
...
Recorded insufficient signal quality per age category. The average FibriCheck benchmark is around ±8%. 
