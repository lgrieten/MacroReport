---
title: "FibriCheck Overview Tergooi"
author: "Fibricheck"
date: "10/03/2022"
output:
...
#remotes::install_github('rstudio/rmarkdown', upgrade = TRUE)
...
#STEP 1: CHANGE TITLE/AUTHOR/DATE IN LINE 1
#STEP 2: CHANGE VARIABLES IN CHUNK "INPUT VARIABLES" STARTING AT LINE 134
#STEP 3: SET EVAL = TRUE IF YOU WANT TO EVALUATE PRESCRIPTIONS AND TRANSMISSIONS OF PATIENTS BETWEEN GROUPS
      #- r load prescriptions dataset
      #- UNCOMMENT: ## Plot the correlation between prescription duration and indication
      #- UNCOMMENT: ## ## Where do my patients come from, and where do they go
#STEP 4: SET THIS VARIABLE TO TRUE IF YOU WANT TO STORE THE INDIVIDUAL RECORDINGS IN PNG FORMAT IN THE FOLDER
export_png <- FALSE
...
#configure input variables
    path <- file.path("/Volumes/GoogleDrive/Shared drives/9. Clinical/9.6 Data reporting/Tergooi/Overview_20220309")
...
delete_users<- c("5ef45ff846e0fb0007fde259","5bf40e5fcff47e0008eb42ae","58074800b2148f3b28ad7590", "5eb1457c4cedfd0006d7a28f", "5db06b8bd6018000069f77ee", "5dc02e7fd6018000069f7be6", "5d39d099c9e77c0006b25ce5","5d808a40c9e77c00089a7846","580747d1b2148f3b28ad7586","5d808a40c9e77c00089a7846","58074800b2148f3b28ad7590","5b3656b9cff47e0006ce3b27")
...
```{r load prescriptions dataset, eval = FALSE, echo=FALSE, message=FALSE}
...
- This report is designed to give you an insight on the selected population you manage with FibriCheck. 
- This report is based on input from our partners and can be improved based on your feedback. Share your ideas to support@fibricheck.com

## High level data overview
The following table shows general metrics calculated from your dataset. 
...
The following section addresses data related to the background of the enrolled participants. Data is coming from the gender.
...
ggplot(plot_left_1_df, aes(x="", y=cnt, fill=gender)) +
  ...
  coord_polar("y", start=0) +  theme_void() + scale_fill_manual(values=c("#1e8d95", "#ffa786")) + geom_text(aes(label = total), position = position_stack(vjust = 0.5), color = "white", size=6) + theme(legend.text=element_text(size=12))
...
```{r}
rbind(`Total number of patients:` = nrow(profile)
      ,`Percentage men:` = percentage_men
      ,`Avg age men:` = round(plot_left_1_df[plot_left_1_df$gender=='male',][['avg']],1)
      ,`Avg age female:` = round(plot_left_1_df[plot_left_1_df$gender!='male',][['avg']],1)
...
The following section addresses data related to the background of the enrolled participants. Data is coming from the registered age.
...
ggplot(plot_right_1_df, aes(x=age, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + ylim(0,(max(plot_right_1_df$cnt)+5)) + xlab("Age groups") + ylab("# subjects") + geom_text(aes(label = cnt), vjust = -0.5, colour = "#1e8d95")
...
```{r}
rbind(`Total number of patients:` = number_of_participants
      ,`Average age:` = average_age
...
Amount of measurements per phone and the distribution of insufficient quality by phone manufacturer
...
o1 <- ggplot(plost_insuffqual, aes(x=device, y=insuff_q, fill="#1e8d95")) + 
     ...
     theme(legend.position = "none") + ylim(0,(max(plost_insuffqual$insuff_q)+50))+
     xlab("Device manufacturers") +
     ylab("% of insuff. quality") +  
     geom_text(aes(label = insuff_q), vjust = -0.5, colour = "#1e8d95")

o2 <-  ggplot(plost_insuffqual, aes(x=device, y=total, fill="#1e8d95")) + 
     geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() +
     theme(legend.position = "none") + ylim(0,(max(plost_insuffqual$total)+50))+
     ylab("# of measurements") +  
     geom_text(aes(label = total), vjust = -0.5, colour = "#1e8d95")
...
## Medical History
- When users have performed their first measurement they are prompted with a screen that allows them to enter a questionnaire. 
- This questionnaire consists of a set of questions that relate to cardiovascular risk factors.
- The questionnaire is optional and participants can skip these questions if they want.
- The table below shows a high-level overview of the cardiovascular risk profile of this population
...
medical_history<-
      ...
      data[which(data == regex("false", ignore_case = TRUE))] <- "0"
      data[which(data == regex("true", ignore_case = TRUE))] <- "1"
      ...
      blank_rows <- sum(rowSums(is.na(data))>2)
      filled_in <-nrow(data) - blank_rows
      filled_in_pct <- round(percent(filled_in/nrow(profile)),3)
      blank_in_pct <- round(percent(blank_rows/nrow(profile)),3)
...
rbind(` Total` = paste(nrow(profile),"patients"),
      ...
      `Amount of filled in questionnaires` = paste(filled_in,"/", filled_in_pct),
      `Amount of blank questionnaires` = paste(blank_rows,"/", blank_in_pct)      ) %>%
    kable() %>%
    kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = T,
...
## Medical History of AFib patients

- When users have performed their first measurement they are prompted with a screen that allows them to enter a questionnaire.
- This questionnaire consists of a set of questions that relate to cardiovascular risk factors.
- The questionnaire is optional and participants can skip these questions if they want.
- The table below shows a high-level overview of the cardiovascular risk profie of patients that had possible atrial fibrillation based on the FibriCheck measurements
...
medical_history_af_2 <- filter(medical_history_af, (id %in% list_user_id_AF$userIds.0))
      ...
      data_af[which(data_af == regex("false", ignore_case = TRUE))] <- "0"
      data_af[which(data_af == regex("true", ignore_case = TRUE))] <- "1"
      ...
      blank_rows_af <- sum(rowSums(is.na(data_af))>2)
      filled_in_af <-nrow(data_af)-blank_rows_af    
      filled_in_pct_af <- round(percent(filled_in_af/nrow(data_af)),3)
      blank_in_pct_af <- round(percent(blank_rows_af/nrow(data_af)),3)
...
rbind(` AF patients` = paste(nrow(list_user_id_AF),"of",nrow(profile),"patients"),
      ...
      `Already takes a bloodthinner` = bloodthinner_af,
       `Amount of filled in questionnaires` = paste(filled_in_af,"/", filled_in_pct_af),
      `Amount of blank questionnaires` = paste(blank_rows_af,"/", blank_in_pct_af) 
      ) %>%
    kable() %>%
    kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
...
- The following data provides insight in the enrollment details of the population. This is relevant information to identify the peak moments when patients activated their prescription. 
...
ggplot(plot_left_2_df, aes(x=date, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + xlab("Date") + ylab("# activations")
```
...
rbind(`Peak activation - day:` = as.character(as.Date(plot_left_2_df %>% arrange(-cnt) %>% head(1) %>% .[[1]],origin='1970-01-01'))
      ,`Peak activation - Time (UTC):` = paste(peck_hour," " ,"h")
      ) %>%
...
- The following data provides insight in the measurement behaviour of the population.
- This is relevant information to identify peak moments when participants performed FibriCheck measurements.
...
ggplot(plot_left_3_df, aes(x=wd, y=cnt_, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 90)) + scale_x_discrete(limits = plot_left_3_df$wd) + xlab("Week") + ylab("% of measurements")  + geom_text(aes(label = cnt_), size = 2, vjust = -0.5, colour = "#1e8d95")
...
ggplot(plot_right_3_df, aes(x=hour_, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + ylim(0,(max(plot_right_3_df$cnt)+20)) +
  scale_x_discrete(limits=plot_right_3_df$hour_)+ xlab("Hour of the day (GMT timezone)") + ylab("# measurements") + geom_text(aes(label = cnt), vjust = -0.5, colour = "#1e8d95")
```
</div>
...
max_measures_per_day <- 
    ...
    ungroup() %>% 
    summarise(max(cnt)) %>% .[[1]]
rbind(`Peak measurements (GMT): ` = plot_right_3_df %>% 
        arrange(-cnt) %>% 
        head(1) %>% 
        .[[1]]
      ,`Total number of measurements received:` = total_number_of_measurements
      ,`Max number of total measurements / day:` = max_measures_per_day
      ) %>%
    kable() %>%
    kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
...
- Users are asked to take `r m_per_day` measurements per day and additional measurements when feeling symptoms.
- Motivation: number of days in which `r m_per_day` measurements were performed per day for the duration of the entire prescribed period. 
- Compliance: number of performed measurements per number of expected measurements over the prescription period (in the manuscript, it called 'adherence').
...
for(i in 1:length(UserID))
{
  ...
  df_prescription = subset(prescriptions,user_id == UserID[i])
    df_prescription = subset(df_prescription,status == "ACTIVATED")
  ...
  df_prescription$activated_timestamp2 = as.Date(as_datetime((df_prescription$update_timestamp/1000))) 
  df_prescription$end_date = as.Date(df_prescription$activated_timestamp2) + df_prescription$dur
  df_prescription$today = as_datetime(Today)
  total_prescription_days = sum(df_prescription$dur)
  Total_days_in_last_block = as.numeric(as.Date(Today)-max(df_prescription$activated_timestamp2))
  ...
  df_window$data.measurement_timestamp <- as.Date(as_datetime((df_window$data.measurement_timestamp/1000)))
  df_window = subset(df_window,data.measurement_timestamp >= min(df_prescription$activated_timestamp2) & data.measurement_timestamp <= max(df_prescription$end_date))
  ...
  compliance = round(100*sum(nrow(df_window))/(Total_days*m_per_day),2)
...
tt1 <- ggplot(motivation_plot_total, aes(x=motivation, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95") +scale_x_discrete(limits=motivation_plot_total$motivation) + theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 30)) + geom_text(aes(label = cnt), vjust = -0.5, colour = "#1e8d95") + ggtitle("Distribution of user motivation") + ylab("Number of patients") + xlab(paste('Motivation of patients (% of days where',m_per_day,'measurements per day were performed)')) + ylim(0, (max(motivation_plot_total$cnt)+10))
...
tt2 <-ggplot(compliance_plot_total, aes(x=compliance, y=cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95") +scale_x_discrete(limits=compliance_plot_total$compliance) +theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 30)) + geom_text(aes(label = cnt), vjust = -0.5, colour = "#1e8d95") + ggtitle("Distribution of user compliance") + ylab("Number of patients") + ylim(0, (max(compliance_plot_total$cnt)+10))
...
Average number of measurements per age category. 
...
ggplot(plot_right_4_df, aes(x=age, y=per_user_cnt, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + geom_text(aes(label = per_user_cnt), vjust = 1.5, colour = "white") + ylab("Average amount of measurements")
```
</div>
<div style="float: right; width: 30%;">
...
This overview covers the findings based on the FibriCheck measurements.
...
rbind(`Amount of people with Afib:` = people_with_atrial_fibrillation
      ,`Amount of atrial fibrillation measurements ` = total_af_measurements
      ,`Amount of people with other cardiac arrhythmias:` = people_with_other_cardiac_arrhythmias
      ,`Average heart rate of non-AF people (bpm):` = mean_HR_non_AF
      ,`Average heart rate during AF (bpm):` = mean_HR_AF
      ,`Measurements with symptoms (%):` = scales::percent(Total_measurements_with_symptoms/as.numeric(total_number_of_measurements))
      ,`Afib measurements with symptoms:` = scales::percent(1-(af_measurements_nosymptoms/total_af_measurements))
...
## Heart rhythm & Symptom findings

Contingency table showing the heart rhythm and symptom correlation.

<!-- <div style="float: left; width: 50%;"> -->

<!-- ```{r, message=FALSE,warning=FALSE, echo=FALSE} -->

<!-- # ggplot(arrhythmia_table, aes(x=`Heart rhythm`, y=`%`, fill="#1e8d95")) + -->
<!-- #   geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_text(aes(label = `%`), vjust = -1, colour = "black") + ylab("Prevalence of arrhythmia's (%)") -->
...
```{r}
plot_rhythm_symptom_correlation <-
   measurments %>% 
    select(data.diagnosis.label.0, data.context.symptoms.0)  %>%
    group_by(Rhythm=data.diagnosis.label.0,Symptoms = data.context.symptoms.0 ) %>% 
    summarise(cnt=n())  %>%
    ungroup()  %>% 
    arrange(Symptoms)
plot_rhythm_symptom_correlation[is.na(plot_rhythm_symptom_correlation)] <- 0

ggplot(plot_rhythm_symptom_correlation, aes(Rhythm, Symptoms)) + geom_point(aes(size = cnt), colour = "#1e8d95") +theme_bw() + scale_size_continuous(range=c(2,19)) + geom_text(aes(label = cnt),colour="white") + xlab("Rhythm Symptom correlation") + theme(legend.position = "none",  axis.text.x = element_text(angle = 90, vjust=0.8))
...
```{r}
plot_rhythm_symptom_correlation2 <-
  measurments %>% 
  select(data.diagnosis.label.0, data.context.symptoms.0)  %>%
  group_by(Rhythm=data.diagnosis.label.0,Symptoms = data.context.symptoms.0 ) %>% 
  summarise(cnt=n())  %>%
  spread(Symptoms, cnt)
plot_rhythm_symptom_correlation2[is.na(plot_rhythm_symptom_correlation2)] <- 0

names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "V1"] <- "Blank"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "chest_pains"] <- "Chestpain"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "fatigue"] <- "Fatigue"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "lightheaded"] <- "Lightheaded"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "no_symptoms"] <- "No symptoms"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "other"] <- "Other"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "palpitations"] <- "Palpitations"
names(plot_rhythm_symptom_correlation2)[names(plot_rhythm_symptom_correlation2) == "shortness_of_breath"] <- "Shortness of breath"



kable(plot_rhythm_symptom_correlation2, align = 'c') %>%
  kable_styling(
    bootstrap_options = c("hover", "condensed", "responsive"),
    full_width = F,
    font_size = 14,)
...
kable(symptoms_table_sinus,caption = "Symptoms during sinus rhythms", align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
```
## Symptoms for non regular rhythms
...
names(symptoms_table_non_af)[names(symptoms_table_non_af) == "fraction"] <- "%"
 ...
 kable(symptoms_table_non_af, caption = "Symptoms during non-AF rhythms", align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
kable(symptoms_table_af,caption = "Symptoms during AF rhythms" , align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
## Prescription types and durations 
...
names(plot_prescriptions_duration)[names(plot_prescriptions_duration) == "cnt"] <- "Amount"
names(plot_prescriptions_duration)[names(plot_prescriptions_duration) == "type"] <- " Duration"



kable(plot_prescriptions_duration, align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
<!-- ## Plot the correlation between prescription duration and indication -->
...
- When people are using FibriCheck there is an important element related to the usability of the application.
- The measurements conducted with the smartphone are sensitive to possible sources of noise.
- If this occurs the signals cannot be analysed for diagnostic purposes. On average, the insufficient quality range on a population is around 8%
```{r}
...
ggplot(measurerement_profiles_fin, aes(x=age, y=mean_Q, fill="#1e8d95")) +
  geom_bar(stat="identity", fill = "#1e8d95")+theme_minimal() + theme(legend.position = "none") + xlab("Age groups") + ylab("Average % insufficient quality recordings")  + ylim(0, (max(measurerement_profiles_fin$mean_Q)+5)) + geom_text(aes(label = round(mean_Q,1)), vjust = -0.5, colour = "#1e8d95")
...
- The measurement attempts reflect how many times a measurement was restarted by the user before it was successful.
A possible reason for insufficient quality could be related to the contextual information.
- The table on the right illustrates the relation between the total amount of insufficient signal quality compared to the context activity the users indicated when the measurement was conducted
...
kable(meas_attempt_measurement_subset, align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
kable(percent_quality_activity, align = 'c') %>%
      kable_styling(
        bootstrap_options = c("hover", "condensed", "responsive"),
        full_width = F,
        font_size = 14)
...
UserID_rand <- sample(1:length(UserID), 25, replace=T)
for(i in 1:length(UserID_rand))
{
df_window = subset(measurments,userIds.0 == UserID[i])
df_window = select(df_window, id, data.measurement_timestamp, data.heartrate,data.indicator,data.context.symptoms.0, data.context.symptoms.sym)
df_window$symptoms <- as.numeric(ifelse((df_window$data.context.symptoms.sym == "NA")|(df_window$data.context.symptoms.sym == "0"),"0", "200"))
...
p1 <- ggplot(df_window) +
          ...
          geom_point(aes(x=data.measurement_timestamp, 
                         y=symptoms), shape=17 , fill = "lightgray",
             color = "black", size = 3 ) + 
...
print(p1)
    ...
    if(export_png == TRUE)
    {
      ggsave(p1, file=paste(UserID[i],".png"))
    }
}
```
